...
export default function StickyCTA({ label, message }) {
  const { t } = useLanguage();
    ...
    <div className="fixed bottom-0 left-0 right-0 z-40 md:hidden">
      <div className="bg-white/95 backdrop-blur-md border-t border-gray-200 px-4 py-3 flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-sm font-semibold text-gray-900 truncate">{label || t('sticky.label')}</div>
          <div className="text-xs text-gray-500">{t('sticky.sub')}</div>
        </div>
        <a
          href={href}
          target="_blank"
